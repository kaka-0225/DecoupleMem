				       ...
				       pg_data_t *pgdat, bool promotion)
{
	...
	/* 迁移追踪: 记录每个页面的虚拟地址和方向，用于迁移正确率分析
	 * MAP_ANONYMOUS: page->index = vaddr >> PAGE_SHIFT
	 * 所以 vaddr = page->index << PAGE_SHIFT */
	{
		struct page *__page;
...
re_cooling:
	...
	trace_printk(
		"[COOLING_DONE] ts=%llu node=%d active_thres=%u warm_thres=%u\n",
		ktime_get_real_seconds(), pgdat->node_id,
