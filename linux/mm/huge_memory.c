...
#ifdef CONFIG_HTMM
	{
		if (memcg->htmm_enabled) {
			struct mem_cgroup_per_node *pn =
				memcg->nodeinfo[page_to_nid(page)];
		      ...
		      pmd_t *pmd, int flags)
{
	...
	if (pmdp_set_access_flags(vma, addr & HPAGE_PMD_MASK, pmd, _pmd,
				  flags & FOLL_WRITE))
		update_mmu_cache_pmd(vma, addr, pmd);
}
		      ...
		      pud_t *pud, int flags)
{
	...
	if (pudp_set_access_flags(vma, addr & HPAGE_PUD_MASK, pud, _pud,
				  flags & FOLL_WRITE))
		update_mmu_cache_pud(vma, addr, pud);
}
...
vm_fault_t do_huge_pmd_numa_page(struct vm_fault *vmf)
{
	...
	target_nid = numa_migrate_prep(page, vma, haddr, page_nid, &flags);
				    ...
				    unsigned long haddr, bool freeze)
{
	...
	if (PageHtmm(&page[3])) {
		...
		for (i = 0, addr = haddr; i < HPAGE_PMD_NR;
			...
			/* Phase 1.E.split: 把 may_hot 标志一起带过去, 否则 split 后
		 * 所有 base page 的 may_hot 都是 false, khugepaged 的
		 * scan_pmd 直接拒收 (line 1334), 永远不会再 collapse 回 THP.
		 * 同步对每个 base page SetPageHtmm, 让 cooling/sampler 能识别. */
			pte_pginfo->may_hot = tail_pginfo->may_hot;
			SetPageHtmm(&page[i]);
				   ...
				   struct list_head *list)
{
	...
	page_tail->flags |=
		(head->flags & ((1L << PG_referenced) | (1L << PG_swapbacked) |
				    ...
				    pgoff_t off_end)
{
	...
	for (index = off_start; index < off_end; index += nr_pages) {
		struct page *fpage = pagecache_get_page(
			mapping, index, FGP_ENTRY | FGP_HEAD, 0);
