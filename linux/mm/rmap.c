...
#include <linux/mempolicy.h> /* Phase 1.B: htmm_cxl_mode */
#include <linux/node.h> /* Phase 1.B: node_is_toptier */
#endif
	...
	while (page_vma_mapped_walk(&pvmw)) {
		...
		if (pvmw.pte) {
			...
			if (memcg_cclock > pginfo->cooling_clock) {
				...
				/* Phase 1.B: 镜像维护双 hist (cooling 全量重建) */
				{
					bool on_dram =
						htmm_cxl_mode ?
							(page_to_nid(page) ==
							...
							node_is_toptier(
								page_to_nid(
									page));
					if (on_dram)
						hca->memcg->dram_hotness_hg
							[cur_idx]++;
					else
						hca->memcg->nvm_hotness_hg
							[cur_idx]++;
				}
