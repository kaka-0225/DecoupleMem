...
static bool __cooling(struct mm_struct *mm, struct mem_cgroup *memcg)
{
	...
	smp_mb();
	trace_printk(
		"[COOLING] ts=%llu clock=%lu nr_sampled=%lu active_thres=%u bp_thres=%u warm_thres=%u max_dram_pages=%lu\n",
		ktime_get_real_seconds(), memcg->cooling_clock,
		memcg->nr_sampled, memcg->active_threshold,
				      ...
				      struct mem_cgroup *memcg)
{
	...
	trace_printk(
		"[THRESHOLD] ts=%llu active_thres=%u bp_thres=%u warm_thres=%u "
		"cooled=%d hg=[%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu] max_dram=%lu\n",
		ktime_get_real_seconds(), memcg->active_threshold,
		memcg->bp_active_threshold, memcg->warm_threshold,
		memcg->cooled, memcg->hotness_hg[0], memcg->hotness_hg[1],
		memcg->hotness_hg[2], memcg->hotness_hg[3],
		memcg->hotness_hg[4], memcg->hotness_hg[5],
		memcg->hotness_hg[6], memcg->hotness_hg[7],
		memcg->hotness_hg[8], memcg->hotness_hg[9],
		memcg->hotness_hg[10], memcg->hotness_hg[11],
		memcg->hotness_hg[12], memcg->hotness_hg[13],
		memcg->hotness_hg[14], memcg->hotness_hg[15],
		memcg->max_nr_dram_pages);
}
...
void update_pginfo(pid_t pid, unsigned long address, enum events e)
{
	...
	if (memcg->nr_sampled % htmm_cooling_period == 0 ||
		...
		trace_printk(
			"[COOLING_TRIGGER] ts=%llu reason=%s nr_sampled=%lu period=%u\n",
			ktime_get_real_seconds(),
			(memcg->nr_sampled % htmm_cooling_period == 0) ?
				"periodic" :
				"alloc_pressure",
			memcg->nr_sampled, htmm_cooling_period);
